...
class MovieTicketTransaction extends Thread{
	...
	MovieTicketTransaction(String email, MovieTicket ticket, boolean prime){
		this.email = email;
		this.ticket = ticket;
		this.prime = prime;
	}
	...
	public void run() {
		...
		synchronized (ticket) {
			...
			if(!prime) {
				try {
					ticket.wait(); // Other Threads can now access the same object and current thread will wait
//					ticket.wait(1000);
			...
			if(ticket.isTicketAvailable()) {
				...
				ticket.notify(); // Notify any thread waiting on ticket to continue its execution
				//ticket.notifyAll(); // if we will have multiple threads waiting :)
			}else {
				System.out.println("Sorry, "+email+" ticket is sold :(");
			}
		}
	}
}
...
class User{
	...
	void selectAndBookMovieTicket(MovieTicket ticket) {
		MovieTicketTransaction transaction = new MovieTicketTransaction(email, ticket, isUserPrime);
		transaction.start();
	}
...
public class WaitNotify {
	...
	public static void main(String[] args) {
		...
		user2.isUserPrime = true; // Fionna upgraded to prime customer 
